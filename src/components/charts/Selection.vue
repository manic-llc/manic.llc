<template>
  <div class="selection-container" ref="container" :class="{ clickable }">
    <BaseCanvas name="selection" ref="canvas" @pointerdown="mouseDown" @pointermove="mouseMove" @pointerup="mouseUp" />
  </div>
</template>
...
import { ScaleRecord } from "../../util/charts"
...
const range: ComputedRef<any> = computed(() => {
  ...
  return coords.value.map(([x]) => transform.value.rescaleX(scales.value.x).invert(x));
});
...
function createGradient(ctx: Ref<CanvasRenderingContext2D>) {
  ...
  const gradient = ctx.value.createLinearGradient(width.value / 2, 0, width.value / 2, height.value);
  gradient.addColorStop(0, `rgba(${base}, 0)`);
  gradient.addColorStop(1, `rgba(${base}, .5)`);
...
function mouseUp(e: MouseEvent) {
  ...
  if (coords.value[1][0] - coords.value[0][0] > 3) $emit("select-end", range.value);
  coords.value = null;
  selecting.value = false;
}
